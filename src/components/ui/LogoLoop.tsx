...
const useResizeObserver = (
  callback: () => void,
  refA: React.RefObject<Element | null>,
  refB?: React.RefObject<Element | null>
) => {
  useEffect(() => {
    ...
    const refs = [refA, refB].filter(Boolean) as Array<React.RefObject<Element | null>>;
  ...
  useEffect(() => {
    ...
    const rootClasses = useMemo(
      () =>
        cx(
          ...
          fadeOut &&
            '[@supports(-webkit-mask-image:linear-gradient(#000,#000))]:[-webkit-mask-image:linear-gradient(to_right,transparent_0%,#000_10%,#000_90%,transparent_100%)]',
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: React.Key) => {
        const isNodeItem = (i: LogoItem): i is NodeLogoItem => 'node' in i;
        ...
        const href = isNodeItem(item) ? item.href : item.href;
