...
export const Card = forwardRef<HTMLDivElement, CardProps>(({ customClass, ...rest }, ref) => (
  <div
    ref={ref}
    {...rest}
    className={`absolute top-1/2 left-1/2 rounded-2xl border border-ui bg-background shadow-xl [transform-style:preserve-3d] [will-change:transform] [backface-visibility:hidden] ${customClass ?? ''} ${rest.className ?? ''}`.trim()}
  />
));
...
const CardSwap: React.FC<CardSwapProps> = ({
  ...
  // Observe viewport visibility and pause/resume accordingly
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const swap = () => {
      ...
      tl.to(elFront, {
        ...
        ease: config.ease as gsap.EaseFunction | string
      });
    ...
    if (pauseOnHover) {
      ...
      const resume = () => {
        if (!inView) return; // don't resume if not visible
        tlRef.current?.play();
        intervalRef.current = window.setInterval(swap, delay);
      };
  ...
  }, [cardDistance, verticalDistance, delay, pauseOnHover, skewAmount, easing, refs, config, inView]);
